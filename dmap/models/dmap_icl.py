...
class DMAPPolicyModelICL(TorchModelV2, nn.Module):
    """Copy of the policy model of DMAP. It defines one policy network for each action component. Each policy
    network can focus on a different part of the proprioceptive state history to generate an 
    embedding for decision making. This branch modifies the attention by freezing it after a few steps, checking the
    ICL hypothesis.
    """
    ...
    def get_keys_and_values(self, adapt_input):
        ...
        if self.steps <= self.steps_bf_freezing:
            ...
            if self.steps == self.steps_bf_freezing:
                print('---------------------------------------------------------------------------')
                print(f"Freezing the attention encoding after {self.steps_bf_freezing} steps!")
                print('---------------------------------------------------------------------------')
